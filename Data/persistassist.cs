...
namespace TESVSnip.Data
{
	...
	{
		object _root;
		PersistContextStates _context;
		int _savecount;
		Hashtable _infoTypeCache = null;
		ArrayList _postSerializeCallbacks = null;
		...
		{
			Type t = obj.GetType();
			Hashtable objmap = _infoTypeCache[ t ] as Hashtable;
			if ( objmap == null )
			{
				objmap = new Hashtable();
				_infoTypeCache[ t ] = objmap;
			}
			SerializationInfoCacheList data = objmap[ obj ] as SerializationInfoCacheList;
			if ( data == null )
			{
		...
		public bool RetrieveCacheInfo( object obj, SerializationInfo info )
		{
			...
			if ( objmap != null )
			{
				...
				if ( data != null )
				{
					...
					while ( itr.MoveNext() )
					{
						SerializationInfoCache cache = itr.Current as SerializationInfoCache;
						if ( cache != null )
	...
	public sealed class PersistableAttribute : System.Attribute
	{
...
//	}
	...
	{
		static Hashtable _PersistableAttributeOnTypeCache = new Hashtable();
		...
		private static void SetDefaultValue(object obj, PropertyInfo pi, bool force)
		{
			try
			{
				...
				if (attrs.Length != 0)
				{
					PersistableAttribute pattr = attrs[0] as PersistableAttribute;
				...
				while ( type != typeof(object))
				{
					foreach (FieldInfo fi in type.GetFields(bi))
					{
						...
						if ( modSearchType == PersistType.None || ((pattr.Flags & modSearchType) == modSearchType) )
						{
							...
							{
								if (fi2.Name == fi.Name) { found = true; break; }
							}
		...
		public static void DeserializationFromSurrogate( object obj, object values )
		{
			...
			System.Collections.Specialized.ListDictionary dict = values as System.Collections.Specialized.ListDictionary;
