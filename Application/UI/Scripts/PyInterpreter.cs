...
namespace FalloutSnip.UI.Scripts
{
    ...
    {
        private static readonly string ScriptsPyPath = Path.Combine(Folders.ScriptsDirectory, @"scripts.py");
        ...
        public static void LoadAllPrototypeFunction()
        {
            ...
            if (_pyDictionary.Count == 0)
                Alerts.Show("@The 'scripts.py' file is empty !", @"TESSnip Error");
        }
        ...
        public static T ExecuteFunction<T>(Domain.Model.Element elemt, FunctionOperation funcOp) where T : struct
        {
            ...
            if (!string.IsNullOrWhiteSpace(p))
            {
                ...
                {
                    Alerts.Show(string.Format(@"ExecuteReadingFunction: {0}", ex.Message), @"TESSnip Error");
                    throw;
                }
            }
                ...
                var el = new PyElement
                             {
                                 ...
                                 Optional = elemt.Structure.optional > 0,
        ...
        public static T ExecuteFunction<T>(ElementStructure elemt, object value, FunctionOperation funcOp) where T : struct
        {
            ...
            if (!string.IsNullOrWhiteSpace(p))
            {
                ...
                {
                    Alerts.Show(string.Format(@"ExecuteWritingFunction: {0}", ex.Message), @"TESSnip Error");
                    throw;
                }
            }
