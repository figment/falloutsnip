...
class HTMLRenderer(): 
	...
	def GetHeaderRecord(self, rec):
		...
		with p.table( id = 'record-hdr' ):
			...
			with p.tbody():
				...
				type = rec.Name
				ok, structure = RecordStructure.Records.TryGetValue(rec.Name)
				if ok: type = "%s: %s"%(rec.Name, structure.description)
	...
	def GetDescriptionRecord(self, rec):
		...
		try:
			...
			p.p('Warning: An error occurred while processing the record. It may not conform to the structure defined in RecordStructure.xml', class_='danger')
			p.p(str(e))
			print e
			import sys,traceback
			traceback.print_exc(file=sys.stdout)
						...
						with p.tr():
							...
							if sselem.type == ElementValueType.Blob:
								...
								if hasOptions:
									...
									sb = StringBuilder()
									for k in xrange(0,sselem.flags.Length,2):
										if ((intVal & (1 << k)) != 0):
											if (sb.Length > 0):
												sb.Append("<br/>")
	...
	def GetDescriptionGroupRecord(self, rec):
		...
		try:
			...
			if rec.groupType == 0:
				type = Encoding.Instance.GetString(data,0,4)
				ok, structure = RecordStructure.Records.TryGetValue(type)
